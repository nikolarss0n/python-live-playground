...
- [x] Run with model on AI prompt-template lesson
- [x] Auth header Web API lesson + companion `GET /me`
- [x] **OpenAPI import** — load companion `/openapi.json`, probe routes, insert Python request dicts

## Left to do

| Priority | Item | Notes |
|----------|------|--------|
| Optional | **runWithModel on more lessons** | Flag exists; only prompt-template uses it |
| Polish | **More compare pairs** | Sparse outside repair labs |
